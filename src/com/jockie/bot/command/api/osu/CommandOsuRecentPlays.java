...
public class CommandOsuRecentPlays extends CommandImpl {
	
	public CommandOsuRecentPlays() {
		super("osu recent plays", CommandOsuUser.ARGUMENT_OSU_MODE, new ArgumentString("Osu name/id"));
		super.setCommandDescription("Get best plays from an osu user");
		super.setBeta(true);
	}
	...
	public void execute(MessageReceivedEvent event, String prefix, Object... arguments) {
		...
		ArrayList<OsuPlay> plays = Osu.getRecentPlays(user, mode);
		if(plays.size() == 1) {
			...
			event.getChannel().sendMessage("No recent plays by " + user).queue();
		}
	}
}
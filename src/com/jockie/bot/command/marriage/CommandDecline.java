...
public class CommandDecline extends CommandImpl {

	public CommandDecline() {
		super("decline");
		super.setBotTriggerable(false);
		super.setCommandDescription("Declines the current marriage proposal");
	}
			...
			if(!Boolean.parseBoolean((String) author_row.getColumn(PersonColumn.MARRIED.getValue()))) {
				if(author_status.equals(Propose.PROPOSEDTO)) {
					Utility.getUser(event.getJDA(), event.getGuild(), author_partner, partner_name -> {
						Marriage.removeProposal(author_partner, user_author.getId());
						event.getChannel().sendMessage(user_author.getAsMention() + " just declined the proposal to marry " + partner_name).queue();
					});
				}else{
